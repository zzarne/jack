...
def consistency_check(cf):
    ...
    if 'freedb_server' in cf:
        if cf['freedb_server']['val'] not in jack_freedb.freedb_servers:
            error("unknown server, choose one: " + repr(list(jack_freedb.freedb_servers.keys())))
    ...
    if cf['_encoder'] not in jack_helpers.helpers or jack_helpers.helpers[cf['_encoder']]['type'] != "encoder":
        dummy = []
        for i in list(jack_helpers.helpers.keys()):
            if jack_helpers.helpers[i]['type'] == "encoder":
                dummy.append(i)
        error("Invalid encoder, choose one of " + ", ".join(dummy))

    if cf['_ripper'] not in jack_helpers.helpers or jack_helpers.helpers[cf['_ripper']]['type'] != "ripper":
        dummy = []
        for i in list(jack_helpers.helpers.keys()):
    ...
    if cf['_vbr'] and 'vbr-cmd' not in jack_helpers.helpers[cf['_encoder']]:
        warning("disabling VBR because " + cf['_encoder'] + " doesn't support it.")
        cf.rupdate({'vbr': {'val': 0}}, "check")

    if cf['_otf']:
        for i in (cf['_ripper'], cf['_encoder']):
            if ('vbr-' * cf['_vbr'] * (i == cf['_encoder'])) + 'otf-cmd' not in jack_helpers.helpers[i]:
                error("can't do on-the-fly because " + jack_helpers.helpers[i]['type'] + " " + i + " doesn't support it.")

    if not cf['_vbr'] and 'cmd' not in jack_helpers.helpers[cf['_encoder']]:
...
def check_rc(cf, global_cf, user_cf, argv_cf):
    ...
    if 'ripper' not in all_keys:
        ...
        if not jack_utils.in_path(default_ripper):
            rippers = [i for i in jack_helpers.helpers if jack_helpers.helpers[i]["type"] == "ripper" and "toc_cmd" in jack_helpers.helpers[i]]
            for cmd in rippers:
