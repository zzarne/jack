...
def choose_cat(cat = ["blues", "classical", "country", "data", "folk", "jazz", "misc", "newage", "reggae", "rock", "soundtrack"]):
    ...
    while x < 0 or x > len(cat):
        if 'freedb_cat' in jack_progress.status_all and jack_progress.status_all['freedb_cat'][-1] in cat:
            input = input(" 0.) none of the above (default='%s'): " % jack_progress.status_all['freedb_cat'][-1])
            if not input:
