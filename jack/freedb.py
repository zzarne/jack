...
# renamed.
            ...
            if jack.utils.check_path(dirs_created, old_dirs) and not jack.utils.check_path(dirs_created, new_dirs):
                ...
                jack.functions.progress("all", 'ren', (dir_created + "-->" + new_dir).encode(cf['_charset'], "replace"))
    if not err:
        cd = track_names[0]
        year = genretxt = None
        if len(cd) > 2:
...
def freedb_query(cd_id, tracks, file):
    ...
    if buf and buf[0:1] == "2":
        if buf[0:3] in ("210", "211"):  # Found inexact or multiple exact matches, list follows
            ...
            if cf['_cont_failed_query']:
                warning(buf + f.read().decode() + " How about trying another --server?")
                err = 1
                return err
            else:
                error(buf + f.read().decode() + " How about trying another --server?")
        else:
            if cf['_cont_failed_query']:
                warning(buf + f.read().decode() + " --don't know what to do, aborting query.")
                err = 1
                return err
            else:
...
def choose_cat(cat=["blues", "classical", "country", "data", "folk", "jazz", "misc", "newage", "reggae", "rock", "soundtrack"]):
    ...
    while x < 0 or x > len(cat):
        if 'freedb_cat' in jack.progress.status_all and jack.progress.status_all['freedb_cat'][-1] in cat:
            input = input(" 0.) none of the above (default='%s'): " % jack.progress.status_all['freedb_cat'][-1])
            if not input:
...
def do_freedb_submit(file, cd_id, cat=None):
    ...
    if not cat:
        hello = "hello=" + cf['_username'] + " " + cf['_hostname'] + " " + jack.version.name + " " + jack.version.version
        ...
        if buf[0:3] == "500":
            ...
            error("LSCAT failed: " + buf.rstrip() + f.read().decode())
    ...
    if cf['_debug']:
        ...
        h.putheader('X-Cddbd-Note', 'data submitted with ' + jack.version.name + ' (' + jack.version.url + ')')
    ...
    if proxy:
        if err != 200:
            error("proxy: " + repr(err) + " " + msg + f.read().decode())
        else:
            buf = f.readline().decode()
...
def update_revision(file):
    ...
    for line in freedb_file.readlines():
        ...
        if rev:
            ...
            os.write(tmp, "# Submitted via: " + jack.version.name + " " + jack.vorsion.version + "\n")
        else:
