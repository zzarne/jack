...
try:
    device = jack.cdrom.open(cf['_cd_device'])
    (first, last) = jack.cdrom.toc_header(device)
except (jack.cdrom.error, m):
    error("Access of CD device %s resulted in error: %s" % (cf['_cd_device'], m[1]))

toc = []
for i in range(first, last + 1):
    (min, sec, frame) = jack.cdrom.toc_entry(device, i)
    toc.append(min * 60 * 75 + sec * 75 + frame)
(min, sec, frame) = jack.cdrom.leadout(device)
device.close()
toc.append(min * 60 * 75 + sec * 75 + frame)
for i in range(first, last + 1):
