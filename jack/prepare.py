...
def find_workdir():
    ...
    while (not os.path.exists(cf['_toc_file'])) or cf['_multi_mode']:
        ...
        if cf['_guess_mp3s']:
            ...
            my_dirs = []      # dirs containing toc_file
            for i in dirs:
            ...
            if len(unique_dirs) > 1:
                ...
                if cf['_create_dirs']:
                    ...
                    dir_name = jack.version.name + "-" + jack.freedb.freedb_id(jack.ripstuff.all_tracks, warn=0)
                    if not os.path.exists(dir_name) and not os.path.isdir(dir_name):
                        os.mkdir(dir_name)
                    os.chdir(dir_name)
                    jack.freedb.dir_created = dir_name
                    jack.functions.progress("all", "mkdir", jack.freedb.dir_created)
...
def read_toc_file():
    ...
    if os.path.exists(cf['_toc_file']):
        ...
        if not os.path.exists(cf['_def_toc']):
            jack.functions.cdrdao_puttoc(cf['_def_toc'], jack.ripstuff.all_tracks, jack.freedb.freedb_id(jack.ripstuff.all_tracks))
    ...
    if cf['_upd_progress']:
        ...
        for i in todo:
            ...
            if not status[num]['dae']:
                filename = jack.functions.check_file(num, i, ".wav")
                if filename:
                    status[num]['dae'] = "  *   [          simulated           ]"
                    jack.functions.progress(num, "dae", status[num]['dae'])
            if not status[num]['enc']:
                filename = jack.functions.check_file(num, i, ext)
                if filename:
                    if ext.upper() == ".MP3":
                        x = jack.mp3.mp3format(filename + ext)
                        temp_rate = x['bitrate']
...
def read_progress(status, todo):
    ...
    jack.progress.status_all['id3_genre'] = int(jack.progress.status_all['id3_genre'][-1])
    jack.progress.status_all['id3_year'] = int(jack.progress.status_all['id3_year'][-1])
...
def freedb_submit(cat=None):
    ...
    if err:
        ...
        if cf['_freedb_mailsubmit']:
            jack.freedb.do_freedb_mailsubmit(cf['_freedb_form_file'], cd_id, cat)
        else:
        ...
        if os.path.exists(bakfile):
            try:
                ...
                if pdiff:
                    ...
                    print()
                    print(pdiff)
                    x = input("Would you like to submit these changes to the FreeDB server? (y/N) ")
                    if x and x[0].upper() == "Y":
