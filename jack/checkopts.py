...
def consistency_check(cf):
    ...
    if len(jack.freedb.freedb_servers[cf['_freedb_server']]['my_mail']) <= 3 or jack.freedb.freedb_servers[cf['freedb_server']['val']]['my_mail'] == "default":
        tmp_mail = jack.freedb.freedb_servers[cf['freedb_server']['val']]['my_mail']
        tmp_mail2 = cf['_my_mail']
        if len(cf['_my_mail']) <= 3 or cf['_my_mail'] == "default" or cf['_my_mail'].find("@") < 1:
            env = os.getenv("EMAIL")
            if env:
                jack.freedb.freedb_servers[cf['freedb_server']['val']]['my_mail'] = env
            else:
                jack.freedb.freedb_servers[cf['freedb_server']['val']]['my_mail'] = cf['username']['val'] + "@" + cf['hostname']['val']
            if len(cf['my_mail']['history']) > 1:
                warning("illegal mail address changed to " + jack.freedb.freedb_servers[cf['freedb_server']['val']]['my_mail'])
        else:
            jack.freedb.freedb_servers[cf['freedb_server']['val']]['my_mail'] = cf['_my_mail']
